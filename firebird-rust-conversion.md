...
answering one row even over an empty set) and `HAVING` (filtering the
computed groups, including by aggregates not in the select list) — the
...
more column types, DML), not the protocol — so firebird-qa remains
