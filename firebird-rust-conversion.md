...
debugger. What stands between here and
running the suite is now *breadth* — select-list expressions and
arithmetic, DDL beyond plain CREATE TABLE — not the storage engine,
the indexes, the protocol or the catalog: every layer from pages to B-trees to the wire now both
