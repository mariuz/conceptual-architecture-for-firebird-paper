...
table from the literal equivalents. What stands between here and
running the suite is now *breadth* — DDL and the wider expression
surface — not the storage engine, the indexes or the protocol: every layer from pages to B-trees to the wire now both
