...
joins (two relations matched on AND-ed column equalities through qualified
or bare names, NULL keys never joining) — the
...
running the suite is the remaining *breadth* of the SQL surface (more
column types, DML, outer joins), not the protocol — so firebird-qa remains
