...
| Transactions, TIP semantics, snapshots | [transactions-and-concurrency.md](transactions-and-concurrency.md) | `tra.cpp`, `vio.cpp`, `sqz.cpp` | **done, and the server reads through it** — TIP-driven committed-only visibility walk (with delta reconstruction) matches a fresh-snapshot `SELECT` on a file frozen mid-uncommitted-work; the wire server's transactions now stay `tra_active` in the TIP until COMMIT flips the two bits, so a row an attachment has written and not committed is invisible to every other attachment, as on the engine |
...
| Metadata cache | [metadata-cache.md](metadata-cache.md) | `met.epp`, `MetadataCache` | **wired** — what the catalog says about a relation (id, columns, formats, index operations, NOT NULL fields, identity column) is held per database and dropped by DDL, the engine's own invalidation rule; measured with a statement-phase timer, it took an INSERT from 7.4ms to 4.3ms because planning had been re-walking the system relations for every statement |
