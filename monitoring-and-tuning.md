...
- **SQL/PSQL profiler** (Firebird 5) — the `RDB$PROFILER` system package plus `PLG$PROF_*` tables record per-statement and per-record-source timings, so you can see *where inside a plan* the time went. Verified present on the live server (`RDB$PROFILER` package exists). It is the *accumulation* to the `MON$` snapshot and the trace stream, and it gets its own treatment in [the profiler document](profiler.md) — including per-operator timings read back as an indented plan, PSQL hotspots by line and column, profiling another attachment, and the overhead calibration that makes the reported numbers subtract the cost of measuring them.
