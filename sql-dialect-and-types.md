...
### Free Pascal sample — [`samples/fpc/types_demo.pas`](samples/fpc/types_demo.pas)

The same showcase through [fbintf](https://github.com/MWASoftware/fbintf) (vendored at [`extern/fbintf`](extern/fbintf)), MWA Software's Firebird Pascal API — the layer under IBX, driving the same libfbclient behind COM-style reference-counted interfaces (`make -C samples/fpc bin/types_demo && samples/fpc/bin/types_demo`; the file is `types_demo.pas` because a program named `types.pas` would shadow the FPC RTL unit `Types` that fbintf's uses-closure needs, breaking every build — the same clash that named the C++ twin `transactions_demo.cpp`). This is where the wrapper-family ladder tops out: everything the JS and Rust twins could not fetch typed, fbintf hands over as real values — `INT128` and `DECFLOAT(34)` arrive as `TBCD` via `AsBCD` (FmtBCD, Pascal's arbitrary-precision decimal), `BOOLEAN` as a Pascal `boolean`, and `TIMESTAMP WITH TIME ZONE` decodes through `GetAsDateTime` to wall time, DST offset *and* the zone name. `IColumnMetaData` even names the wire codes itself (`GetSQLType` plus `GetSQLTypeName`), and the domain violation is a typed `EIBInterBaseError` with the gds code in `IBErrorCode`. One honest asymmetry against the C++ twin: `AsString` formats client-side in `FBSQLData` rather than through the server's CVT rules, so the text rendering follows the client's date format and prints the zone as its offset.
