...
### Free Pascal sample — [`samples/fpc/gc_sweep.pas`](samples/fpc/gc_sweep.pas)

The same experiment through [fbintf](https://github.com/MWASoftware/fbintf) (vendored at [`extern/fbintf`](extern/fbintf)), MWA Software's Firebird Pascal API — the layer under IBX — driving the same libfbclient as the C++ samples behind COM-style reference-counted interfaces (`make -C samples/fpc bin/gc_sweep && samples/fpc/bin/gc_sweep`). Each counter peek is a fresh read-committed transaction (MON$ tables are a stable snapshot per transaction) around a single `OpenCursorAtStart`, single-statement steps collapse to `ExecImmediate([tpb...], sql)` with no explicit transaction object at all — and where rsfbclient's typed `TransactionConfiguration` had no way to express the stump, fbintf's TPB is the same open array of `isc_tpb_*` constants the OO API packs by hand, so the flag just joins the list: `StartTransaction([isc_tpb_write, isc_tpb_nowait, isc_tpb_concurrency, isc_tpb_no_auto_undo], taRollback)` reaches the feature the Rust section could only demonstrate the absence of.
