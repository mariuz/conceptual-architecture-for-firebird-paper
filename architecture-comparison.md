...
### Rust sample — [`samples/rust/src/bin/architecture_comparison.rs`](samples/rust/src/bin/architecture_comparison.rs)

The Rust version through [rsfbclient](https://github.com/fernandobatels/rsfbclient), Rust's Firebird client (`cd samples/rust && cargo run --bin architecture_comparison`), is the one driver that holds both halves of this comparison at once: its native backend links `libfbclient` — Y-valve, providers and all — so `builder_native().with_dyn_link().with_remote()` gives attachment [1] and `.with_embedded()` gives attachment [2], while its pure-Rust backend (`builder_pure_rust()`) is a wire-protocol reimplementation exactly like node-firebird and so reproduces only [1] as attachment [3]. Where the C++ samples show what a native client can do and the JavaScript sample shows what a wire-only client cannot, the Rust sample shows both from the same `SimpleConnection` API — the backend choice, not the query code, decides which rows of the comparison table are reachable. The embedded leg needs no server at all; it creates its scratch database with `create_database()` under the process's own filesystem rights.
