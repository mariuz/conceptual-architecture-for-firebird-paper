...
Firebird includes many tools and utilities for developing and administering a database, and the code base includes code from older versions, as well as some experimental branches (Firebird devel branch: [master](https://github.com/FirebirdSQL/core)). We will not be considering these in our analysis.
...
The remote communication subsystem, REMOTE, allows clients to communicate remotely, or locally, with the server. It enables communication over several different network protocols, such as TCP/IP, [XNET](https://github.com/FirebirdSQL/core/tree/B2_5_Release/src/remote/xnet.cpp) (is firebird implementation of the local transport protocol). The subsystem is split roughly into two parts: a server side, and a client side. It contains generic code for client-server communication, as well as protocol-specific code.
