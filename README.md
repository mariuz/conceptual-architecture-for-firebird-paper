...
> **Note on this revision.** The diagrams below were originally raster images (GIF/PNG); they have been re-drawn as inline [Mermaid](https://mermaid.js.org/) diagrams so they render directly on GitHub and stay editable in text. The source-tree links have been updated from the legacy `B2_5_Release` branch of the `core` mirror to the current [Firebird 6 `master` branch](https://github.com/FirebirdSQL/firebird). Runnable C++ examples that exercise the architecture described here live in [`samples/`](samples/), and a new section, [Architectural evolution: Firebird 3 to 6](#architectural-evolution-firebird-3-to-6), tracks how the architecture has changed since the paper was written. Three companion documents extend it: [Architecture Comparison: Firebird, PostgreSQL, MySQL and SQLite](architecture-comparison.md) places Firebird's architecture side by side with three other open-source database systems; [The Firebird 6 Wire Protocol and SRP Authentication](firebird-wire-protocol.md) documents the REMOTE subsystem's on-the-wire protocol (with a comparison to the PostgreSQL and MySQL protocols) and runnable Node.js and C++ examples; [Embedded Architecture: Firebird vs SQLite](embedded-architecture-comparison.md) contrasts the two embedded database architectures; [Replication Architecture](replication-architecture.md) traces Firebird's replication evolution across versions 3–6 and compares it with PostgreSQL, MySQL and SQLite, with a validated setup walk-through; [On-Disk Structure](on-disk-structure.md) documents the Firebird page format (ODS 14) from the source and `gstat`, with its advantages over PostgreSQL's WAL, MySQL's InnoDB and SQLite; [Security Architecture](security-architecture.md) compares authentication, wire and at-rest encryption, and authorization across the four systems; [Monitoring and Performance Tuning](monitoring-and-tuning.md) covers the MON$ tables, trace, profiler, query plans and the configuration knobs, compared with PostgreSQL, MySQL and SQLite; [Backup and Recovery](backup-and-recovery.md) covers `gbak`, `nbackup`, crash recovery without a log, and validation, compared with the other three systems; [High Availability and Clustering](high-availability.md) covers shadows, replication-based standbys, failover and how the four systems approach clustering; [SQL Dialect and Data Types](sql-dialect-and-types.md) compares Firebird's SQL dialects and type system (INT128, DECFLOAT, time zones) with PostgreSQL, MySQL and SQLite; [SQL Grammar and Parser](grammar-and-parser.md) provides a full, regenerable Mermaid diagram of the Firebird grammar (with the generator script) and compares the parser architectures; [Query Optimizer and Execution Engine](query-optimizer-and-execution.md) covers Firebird's cost-based optimizer, access paths, join methods and the Volcano record-source executor, with real plans, compared with PostgreSQL, MySQL and SQLite; [Transactions, Concurrency and Isolation Levels](transactions-and-concurrency.md) covers the multi-generational model, the three isolation levels, commit-order snapshots and conflict handling, with live demos, compared across the four systems; [PSQL, Stored Procedures and Triggers](psql-and-stored-procedures.md) covers Firebird's procedural language — selectable procedures, packages, and DML/DDL/database triggers — with side-by-side examples in PL/pgSQL, MySQL and SQLite; [Extensibility](extensibility.md) covers the plugin architecture, external engines and UDR (with a live-run native routine), compared with PostgreSQL extensions, MySQL storage engines and SQLite virtual tables; [Internationalization](internationalization.md) covers the INTL subsystem — character sets, ICU-backed collations (case/accent-insensitive), and transliteration — compared across the four systems; [Client APIs and Drivers Across Languages](client-apis-and-drivers.md) covers the OO and ISC C APIs, the native-binding vs pure-protocol driver split, and the per-language driver ecosystem; [Connection Pooling and External Connections](connection-pooling.md) covers the built-in external-connections (EDS) pool, Firebird's inbound-connection story, and the PgBouncer/ProxySQL contrast; [Deployment and Operations](deployment-and-operations.md) covers the install layout, the configuration files, `ServerMode`, aliases and containers, compared across the four systems; and [Migration and Interoperability](migration-and-interoperability.md) covers Firebird version upgrades (ODS/`gbak`), engine-to-engine migration with type mapping, and runtime interoperability (ODBC/JDBC, `EXECUTE STATEMENT ON EXTERNAL`, external-file tables).
...
- **Parallel operations.** The engine can run sweep, index creation, and gbak backup/restore with multiple worker threads, implemented as internal worker attachments controlled by the `ParallelWorkers`/`MaxParallelWorkers` settings (see [`doc/README.parallel_features`](https://github.com/FirebirdSQL/firebird/blob/master/doc/README.parallel_features)).
- **Compiled statement cache.** Compiled requests are now cached and shared per attachment, visible through the new `MON$COMPILED_STATEMENTS` table; repeated preparation of the same SQL skips the DSQL-to-BLR-to-execution-tree pipeline described above.
- **SQL/PSQL profiler.** A profiler implemented as a plugin plus the `RDB$PROFILER` system package records per-statement and per-record-source timings into `PLG$PROF_*` tables.
