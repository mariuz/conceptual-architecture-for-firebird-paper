...
A short, deep-cut companion to the [request trace](request-lifecycle-code-trace.md) (MET/MetadataCache), the [on-disk structure document](on-disk-structure.md) (page types, record formats) and the [DDL path](request-lifecycle-code-trace.md#stage-7-exe-and-the-ddl-path-into-met) (how the catalog is *modified* once it exists).
