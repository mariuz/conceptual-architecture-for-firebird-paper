...
### Free Pascal sample — [`samples/fpc/plans.pas`](samples/fpc/plans.pas)

The same five experiments through [fbintf](https://github.com/MWASoftware/fbintf) (vendored at [`extern/fbintf`](extern/fbintf)), MWA Software's Firebird Pascal API — the layer under IBX (`make -C samples/fpc bin/plans && samples/fpc/bin/plans`). This is a spot where fbintf reaches what both node-firebird and rsfbclient declared out of reach: `IStatement.GetPlan` requests the plan info items from a prepared statement, so the sample returns to the C++ route — every statement prepared, never executed, plan printed — with no `RDB$SQL.EXPLAIN` detour and no execution side effects. One genuine fbintf quirk narrows it: `GetPlan` hardwires the detailed form (it always calls the OO API's `getPlan` with `detailed = true`), so the terse legacy `PLAN (...)` one-liner the C++ sample also prints remains a C++ exclusive even here.
