...
### Rust sample — [`samples/rust/src/bin/profiler.rs`](samples/rust/src/bin/profiler.rs)

The same session through [rsfbclient](https://github.com/fernandobatels/rsfbclient), Rust's Firebird client (`cd samples/rust && cargo run --bin profiler`) — the third proof that a SQL-package control surface loses nothing in any driver. `START_SESSION` comes back as a one-element tuple from `query_first`, the `PROFILE_ID` is bound as an ordinary `?` parameter, and each view lands in a typed `Vec` of tuples — `(String, i64, i64, i64)` for the operator rows. Two idioms are Rust-specific: the plan tree's indentation is built *in SQL* (`lpad('', level * 2) || cast(access_path as varchar(120))`), casting the blob column to `VARCHAR` on the way so it arrives as a plain `String`; and because rsfbclient's explicit `SimpleTransaction` style mirrors the C++ samples rather than node-firebird's per-query transactions, the autonomous-flush pitfall applies here too — the sample hard-commits and opens a genuinely new transaction before touching `PLG$PROFILER`.

Verified: the identical seven-operator hash-join tree (`Hash Join (inner) (keys: 1, total key length: 4)`, fetch counters 5001/10001) and the same PSQL signatures — line 8 dominating at 197,361 ns over 20,000 executions and the `WHILE` on line 6 counting 20,001 — on a much warmer run than either C++ one (0.2 ms vs 46 ms for line 8), the counters again the reproducible part. One honest wrinkle: every row reports `column_num = 1`, because Rust's string-continuation `\` strips the next line's leading whitespace, so the procedure's source really does start each statement at column 1 — the profiler is measuring the text the engine received, faithfully.

### Free Pascal sample — [`samples/fpc/profiler.pas`](samples/fpc/profiler.pas)

The same session through [fbintf](https://github.com/MWASoftware/fbintf) (vendored at [`extern/fbintf`](extern/fbintf)), MWA Software's Firebird Pascal API — the layer under IBX (`make -C samples/fpc bin/profiler && samples/fpc/bin/profiler`) — the fourth proof that a SQL-package control surface loses nothing in any driver. `RDB$PROFILER.START_SESSION` returns the profile id from an `OpenCursorAtStart` one-liner, both `PLG$PROFILER` views come back through the same `IResultset` as any other query, and the plan tree's indentation is built in SQL with `lpad('', level * 2)` exactly as in the Rust twin. Because fbintf's explicit `ITransaction` style mirrors the C++ samples rather than node-firebird's per-query transactions, the autonomous-flush pitfall applies here too: after `FINISH_SESSION` the sample hard-commits and starts a genuinely new transaction, since a retained SNAPSHOT would never see the plugin's autonomously committed flush.
