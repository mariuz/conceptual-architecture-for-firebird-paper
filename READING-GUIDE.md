...
- **[Parallel Workers](parallel-workers.md)** — Firebird 5's headline engine feature, and a characteristic answer to it: the unit of parallel work is not a thread but a **database attachment**, pooled per database, borrowed and returned — so a worker inherits `tdbb`, a memory pool, lock ownership and a row in `MON$ATTACHMENTS` for free. Two pools (threads via `Coordinator`, attachments via `WorkerAttachment`), a `Task`/`WorkItem` framework where `getMaxWorkers()` lets *the task* propose its own width from the work it actually has, and a pool that **fails rather than waits** at the cap. Proven live by watching three `<Worker>` system attachments appear beside Cache Writer and Garbage Collector for the duration of a `CREATE INDEX` and vanish afterwards. Two honest caveats it does not bury: there is **no parallel query execution** (only sweep and index build), and the test machine had one core, so no scaling result is claimed.
...
- **[The Compiled Statement Cache](statement-cache.md)** — Firebird 5's other cache, and a document about one question a statement cache is forced to answer exactly: *when are two SQL statements the same statement?* The key is built by hand — dialect and charset flags, the text **verbatim** with no normalization, and every entry of the [schema search path](schemas-and-name-resolution.md), because the same text under two paths names two different tables. A **second** key of the caller's granted roles decides whether `verifyAccess()` can be skipped, so the plan is shared across users while the privilege check is re-run once per distinct role-set and memoized. Two lists (active statements are not evictable), a 2 MB LRU budget, and — in deliberate contrast to the fine-grained [metadata cache](metadata-cache.md) — invalidation by sledgehammer: **any DDL commit purges every attachment's entire cache**, measured live at roughly 7× on preparation cost. Also records that `MON$COMPILED_STATEMENTS` is *not* a view of this cache, so the cache is unobservable.
...
- **[The Profiler](profiler.md)** — the third view of a running engine, after the `MON$` *snapshot* and the trace *stream*: **accumulation**, per operator, totalled across executions. RAII stopwatches around every record-source open and fetch and every PSQL line; a `Default_Profiler` plugin that lands the results not in a text log but in a normalized `PLG$PROFILER` schema with foreign keys and aggregating views, because the questions people ask of profiling data are queries. Remote profiling of another attachment uses the lock manager twice without ever wanting exclusion — an `LCK_attachment` probe where *success means the target is dead*, and an `LCK_profiler_listener` doorbell that fires a blocking AST — then talks over shared memory. The headline is the inverse of trace's: where trace refuses to affect the engine, a profiler cannot, so it calibrates the cost of its own clock reads every 30 s and subtracts them from what it reports — with the residual wall-clock cost measured live at ~80% on a worst-case PSQL loop.
...
- **[Debugging Firebird in C++](debugging-firebird.md)** — the shared recipe behind every document's Hands-on section: a symbolled engine built from the vendored submodule in ~13 minutes, the embedded-engine trick that puts the whole engine in your debuggable process (`FIREBIRD=<debug root>` + a local path), a verified gdb session whose backtrace *is* the paper's Figure 1 (y-valve → cloop ABI boundary → `JTransaction` → `TRA_commit`), attaching to a live server, and the honest advice that the engine's own instrumentation (`MON$`, trace, profiler, `fb_lock_print`) is often the better debugger.
...
- **DBA / operations** → [Deployment](deployment-and-operations.md) → [Monitoring & Tuning](monitoring-and-tuning.md) → [Backup & Recovery](backup-and-recovery.md) → [High Availability](high-availability.md) → [Replication](replication-architecture.md) → [Security](security-architecture.md) → [Connection Pooling](connection-pooling.md) → [Trace and Audit](trace-and-audit.md) → [The Profiler](profiler.md).
...
- **Runnable code:** [`samples/`](samples/) — one verified C++ + JavaScript sample pair per subsystem document (each document's **Hands-on** section shows its pair's real output, things to try, and gdb breakpoints into the engine, per the [debugging guide](debugging-firebird.md)) — plus the original OO-API clients, the from-scratch SRP/Arc4 wire handshake, and [`tools/grammar_to_mermaid.py`](tools/grammar_to_mermaid.py) (the grammar-diagram generator), with generated [`diagrams/`](diagrams/).
