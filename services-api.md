...
### Free Pascal sample — [`samples/fpc/services.pas`](samples/fpc/services.pas)

The same session through [fbintf](https://github.com/MWASoftware/fbintf) (vendored at [`extern/fbintf`](extern/fbintf)), MWA Software's Firebird Pascal API — the layer under IBX — and this is the flagship of what the wrapper reaches: where rsfbclient has no Services API at all and node-firebird had to re-implement the service opcodes in JavaScript, fbintf's `IServiceManager` drives the *real* API through libfbclient (`make -C samples/fpc bin/services && samples/fpc/bin/services`). The buffers keep their C-API names behind typed builders: `FirebirdAPI.GetServiceManager('localhost', TCP, SPB)` attaches with credentials in an `ISPB`, an `ISRB` carrying `isc_info_svc_server_version` asks for server facts, a second one carrying `isc_action_svc_backup` + `isc_spb_dbname` / `isc_spb_bkp_file` / `isc_spb_verbose` hands `Svc.Start(Req)` to `BURP_main` on a server thread — both paths server paths — and the ring-buffer drain is the document's polling loop written out: repeated `Svc.Query` calls on an `isc_info_svc_line` SRB until the empty line signals the utility is done.
