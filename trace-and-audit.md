...
### Rust sample — [`samples/rust/src/bin/trace.rs`](samples/rust/src/bin/trace.rs)

The one twin that *cannot* run the session — and that is the section's lesson. [rsfbclient](https://github.com/fernandobatels/rsfbclient), Rust's Firebird client (`cd samples/rust && cargo run --bin trace`), exposes no Services API at all: no service attach, no `isc_action_svc_trace_start`, no `TraceLog` stream — where both C++ twins and node-firebird hold the observer's end of the channel, from Rust that end simply is not reachable (the sample prints the `fbtracemgr -start` command line that would substitute). So it plays the two roles that remain. As the *observed* side it runs the same marker query, so a session started externally will show its `ATTACH`/statement/`DETACH`; and as a poor man's `print_perf` it recovers the counters trace would have logged as before/after deltas from `MON$RECORD_STATS` and `MON$IO_STATS` — monitoring answers "what has accumulated", trace answers "what just happened", and a delta around a single statement lets the first approximate the second.

Verified: the marker `SELECT COUNT(*) FROM RDB$RELATIONS` returns 60, and the MON$ deltas around it read `record seq reads: +60` — the same `Natural = 60` the C++ session's per-table counters captured — plus `record idx reads: +2`, `page reads: +0`, `page fetches: +96`. The honest residue of the substitution is printed with the numbers: these are attachment-level deltas, so the MON$ probe queries' own work is folded in, where a real trace session attributes counters to exactly one statement.

### Free Pascal sample — [`samples/fpc/trace.pas`](samples/fpc/trace.pas)

The counter-arc to the Rust section: where rsfbclient cannot reach the observer's end of the channel at all, [fbintf](https://github.com/MWASoftware/fbintf) (vendored at [`extern/fbintf`](extern/fbintf)) — MWA Software's Firebird Pascal API, the layer under IBX, driving the same libfbclient behind COM-style reference-counted interfaces — runs the full two-service session natively (`make -C samples/fpc bin/trace && samples/fpc/bin/trace`), the only wrapper family beyond C++ that can, and without even fb-cpp's `getHandle()` escape hatch: `IServiceManager` attaches with a typed `ISPB`, service A starts the session with `AllocateSRB` + `isc_action_svc_trace_start` + the configuration text in `isc_spb_trc_cfg`, the stream drains through `Svc.Query` requests answered as `IServiceQueryResults.find(isc_info_svc_line)`, and a second service stops it by id via `isc_spb_trc_id` — the same raw tags, but as methods on wrapper-owned request objects. One FPC-specific wrinkle: `Output` is a threadvar, one buffer per thread, so the reader and the worker must flush inside the critical section or their lines interleave.
