...
### Free Pascal sample — [`samples/fpc/numerics.pas`](samples/fpc/numerics.pas)

The same four experiments through [fbintf](https://github.com/MWASoftware/fbintf) (vendored at [`extern/fbintf`](extern/fbintf)), MWA Software's Firebird Pascal API — the layer under IBX — driving the same libfbclient behind COM-style reference-counted interfaces (`make -C samples/fpc bin/numerics && samples/fpc/bin/numerics`). Where the OO-API twin digs the scaled integer out of the raw message buffer, fbintf hands it over as a first-class object: `ISQLData.GetAsNumeric` returns an `IFBNumeric` whose `getRawValue` is the untouched wire integer and `getScale` the wire scale — and the instructive contrast sits one line below it: `AsInt64` on the same column does *not* give you the raw integer, it **rescales** to a whole number, silently discarding the fraction. `INT128` arrives typed as `TBCD` via `GetAsBCD` — the value the JS and Rust twins could only reach through server-side CAST. One fbintf gap the sample works around: `TBCD` has no representation for DECFLOAT's special values, so an untrapped `Infinity` would decode as 0 — the sample lets the engine render it to VARCHAR instead.
